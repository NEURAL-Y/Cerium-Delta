...
class NVS:
    ...
    - Explicit support for convolutional weight tensors and broad castable
    ...
    - A plugin-style metric API that allows external diagnostic modules
      to register additional score computations and threshold
      functions.

    Development notes
    -----------------
    The current implementation is intentionally minimal in dependencies.
    It only relies on ``numpy`` and ``scipy.stats.percentile score``.
    ...
    4. Apply the corresponding threshold method for percentile ranking:
    ...
    def compute(self, choose_metrics="lcs")->object:
        ...
        metric computation and threshold methods. It determines which
        metric family to compute based on ``choose_metrics`` and ensures
        that any corresponding ranking or threshold step is also
        ...
        ranking or threshold step are computed. When ``"all"`` is
        ...
        if choose_metrics == "lcs_weight":
            self.compute_lcs()
            self.threshold_lcs()
            return self.lcs

        elif choose_metrics == "sensitivity_weight":
            self.compute_sensitivity()
            self.threshold_sens()
            return self.sensitivity_score

        elif choose_metrics == "evolution_weight":
    ...
    def compute_sensitivity(self)->dict:
        ...
        self.sensitivity_score = {"raw_values":{},"norm_values":{}}
        x = {
    ...
    def compute_evolution(self)->dict:
        ...
        self.evolution_scores = {"raw_values":{},"norm_values":{}}
        ...
        for k in trained_weights:
            ...
            if k in reference_weights:
                if epochs!=0:
                    self.evolution_scores["raw_values"][k] = (
                                            (trained_weights[k] - reference_weights[k]) / epochs
                                        )
                    self.evolution_scores["norm_values"][k] = (
                        np.linalg.norm(trained_weights[k] - reference_weights[k]) / epochs
                    )
                else:
                    self.evolution_scores["raw_values"][k] = (
                                            trained_weights[k] - reference_weights[k]
                                        )
                    self.evolution_scores["norm_values"][k] = (
    ...
    def threshold_lcs(self)->None:
        ...
        filtered_layer.update({"norm_values":norm})
    ...
    def threshold_sens(self)->None:
        ...
        scores = list(self.sensitivity_score["norm_values"].values())

        percentile = {}

        for layer, score in self.sensitivity_score["norm_values"].items():
    ...
    def threshold_evolution(self)->None:
        ...
        scores = list(self.evolution_scores["norm_values"].values())
        ...
        rankings.update(
            {
                ...
                for k, v in self.evolution_scores["norm_values"].items()
            }
        )
    ...
    def compute_sensitivity_bias(self)->dict:
        ...
        self.sensitivity_score_bias = {"raw_values":{},"norm_values":{}}
        x = {
            k: v.astype(np.float64)
    ...
    def compute_evolution_bias(self)->dict:
        ...
        self.evolution_scores_bias = {"raw_values":{},"norm_values":{}}
    ...
    def threshold_sens_bias(self)->None:
        ...
        scores = list(self.sensitivity_score_bias["norm_values"].values())

        percentile = {}

        for layer, score in self.sensitivity_score_bias["norm_values"].items():
    ...
    def threshold_evolution_bias(self):
        ...
        scores = list(self.evolution_scores_bias["norm_values"].values())
        ...
        rankings.update(
            {
                ...
                for k, v in self.evolution_scores_bias["norm_values"].items()
            }
        )
