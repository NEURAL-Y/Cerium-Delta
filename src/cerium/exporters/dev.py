...
class bridge:
    ...
    def torch_parameters_classifier(self,name,paramters,reference="current")->object:
        ...
        if reference=="current":
            if name.endswith(".weight"):
                self.nvs_memory["weights"][f"layer {self.layer_current.get("torch_weight_index",0)}"] = paramters
                self.layer_current["torch_weight_index"]+=1
            elif name.endswith(".bias"):
                self.nvs_memory["bias"][f"layer {self.layer_current.get("torch_bias_index",0)}"] = paramters
                self.layer_current["torch_bias_index"]+=1
            else:
                return None
        else:
            if name.endswith(".weight"):
                            self.nvs_memory["weights_train"][f"layer {self.layer_train.get("torch_weight_index",0)}"] = paramters
                            self.layer_train["torch_weight_index"]+=1
            elif name.endswith(".bias"):
                            self.nvs_memory["bias_train"][f"layer {self.layer_train.get("torch_bias_index",0)}"] = paramters
